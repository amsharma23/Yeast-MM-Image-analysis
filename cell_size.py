        ...
        for folders in fovs_folder:
            if(("seg" in folders) and ("im" in folders)):
                for trap_fld in listdir(folders):
                    for imgs in listdir(join(folders,trap_fld)):
                        if (isfile(join(folders,trap_fld,imgs)) and (".tif" in imgs)):
                            im_ar = imread(join(folders,imgs));
                            seg_images[imgs] = im_ar;
...
exp_folder = "/Users/amansharma/Documents/Data/test/20220531_ylb128_gal2p_fluor/FOVs";
traps = True;
trajcs_a ={};
ct = 1;
for fov_flds in listdir(exp_folder):
    if(fov_flds!=".DS_Store" and not(".tif" in fov_flds) and ("FOV0" in fov_flds)):

        img_pth = exp_folder+"/"+fov_flds;#FOV folders
        
        #seg_imgs = get_seg_images(img_pth,traps); #store all segmented images       
        ...
        print(img_pth);
        trajcs = cellf.el_vol(img_pth+"/",traps); #the function will plot the estimated vol vs time as fitted by prolate ellipsoids, and spits back the trajectories
        
        if(trajcs is not None):
            trajcs_a[ct] = [trajcs[key] for key in trajcs.keys()];
        
        print(fov_flds+" "+str(ct));    
        ct+=1;
